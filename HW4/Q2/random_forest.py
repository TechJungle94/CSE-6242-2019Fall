...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        # create sample dataset randomly with the help of np.random
        items = np.random.choice(range(0,len(XX)), n)
        
        for item in items:
            samples.append(XX[item][:-1])
            labels.append(XX[item][-1])  
    ...
    def fitting(self):
        ...
        # train the decision trees by the dataset we generate randomly
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                # if counts is empty, put the max vote into y
                votes1 = []
                for i in range(0,self.num_trees):
                    votes1.append(self.decision_trees[i].classify(record))
                
                counts1 = np.bincount(votes1)
                y = np.append(y, np.argmax(counts1))
